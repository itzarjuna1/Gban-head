...
import config
from Gban_head import app  # changed from SONALI to Gban_head
        ...
        if name.startswith("sud"):
            ...
            if await is_on_off(2):
                return await app.send_message(
                    ...
                    text=f"{message.from_user.mention} just checked sudo list.\nID: {message.from_user.id}\nUsername: @{message.from_user.username}",
                )
            return
        ...
        if name.startswith("inf"):
            ...
            if await is_on_off(2):
                await app.send_message(
                    chat_id=config.LOGGER_ID,
                    text=f"{message.from_user.mention} just started bot to check track info.\nID: {message.from_user.id}\nUsername: @{message.from_user.username}",
                )
    else:
        out = private_panel(_)
